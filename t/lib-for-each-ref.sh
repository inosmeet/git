...
test_atom () {
	...
	$test_do $PREREQ "basic atom: $ref $format" '
		git ${GIT_WHICH_TEST} --format="%($format)" "$ref" >actual &&
	...
	then
		...
		$test_do $PREREQ "basic atom: $ref contents:size" '
			...
			*)
			...
			git ${GIT_WHICH_TEST} --format="%(contents:size)" "$ref" >actual &&
...
test_expect_success 'basic atom: refs/tags/testtag *raw' '
	...
	git ${GIT_WHICH_TEST} --format="%(*raw)" refs/tags/testtag >actual &&
...
test_expect_success 'Check invalid atoms names are errors' '
	test_must_fail git ${GIT_WHICH_TEST} --format="%(INVALID)" refs/heads
'

if test "$GIT_WHICH_TEST" = "refs list"
then
	say "Skipping -h crash test for git refs list"
else
	test_expect_success "${GIT_WHICH_TEST} does not crash with -h" '
		test_expect_code 129 git $GIT_WHICH_TEST -h >usage &&
		test_grep "[Uu]sage: git for-each-ref " usage &&
		test_expect_code 129 nongit git $GIT_WHICH_TEST -h >usage &&
		test_grep "[Uu]sage: git for-each-ref " usage
	'
fi

test_expect_success 'Check format specifiers are ignored in naming date atoms' '
	git ${GIT_WHICH_TEST} --format="%(authordate)" refs/heads &&
	git ${GIT_WHICH_TEST} --format="%(authordate:default) %(authordate)" refs/heads &&
	git ${GIT_WHICH_TEST} --format="%(authordate) %(authordate:default)" refs/heads &&
	git ${GIT_WHICH_TEST} --format="%(authordate:default) %(authordate:default)" refs/heads
'

test_expect_success 'Check valid format specifiers for date fields' '
	git ${GIT_WHICH_TEST} --format="%(authordate:default)" refs/heads &&
	git ${GIT_WHICH_TEST} --format="%(authordate:relative)" refs/heads &&
	git ${GIT_WHICH_TEST} --format="%(authordate:short)" refs/heads &&
	git ${GIT_WHICH_TEST} --format="%(authordate:local)" refs/heads &&
	git ${GIT_WHICH_TEST} --format="%(authordate:iso8601)" refs/heads &&
	git ${GIT_WHICH_TEST} --format="%(authordate:rfc2822)" refs/heads
'

test_expect_success 'Check invalid format specifiers are errors' '
	test_must_fail git ${GIT_WHICH_TEST} --format="%(authordate:INVALID)" refs/heads
'

test_expect_success 'arguments to %(objectname:short=) must be positive integers' '
	test_must_fail git ${GIT_WHICH_TEST} --format="%(objectname:short=0)" &&
	test_must_fail git ${GIT_WHICH_TEST} --format="%(objectname:short=-1)" &&
	test_must_fail git ${GIT_WHICH_TEST} --format="%(objectname:short=foo)"
'
...
test_bad_atom () {
	...
	$test_do $PREREQ "err basic atom: $ref $format" '
		test_must_fail git ${GIT_WHICH_TEST} \
...
test_expect_success 'Check format "relative" date fields output' '
	f=relative &&
	(git ${GIT_WHICH_TEST} --shell --format="%(refname) %(committerdate:$f) %(authordate:$f)" refs/heads &&
	git ${GIT_WHICH_TEST} --shell --format="%(refname) %(taggerdate:$f)" refs/tags) >actual
'
...
test_expect_success 'Check format "relative-local" date fields output' '
	test_date relative-local \
		"$(git ${GIT_WHICH_TEST} --format="%(committerdate:relative)" refs/heads)" \
		"$(git ${GIT_WHICH_TEST} --format="%(authordate:relative)" refs/heads)" \
		"$(git ${GIT_WHICH_TEST} --format="%(taggerdate:relative)" refs/tags)"
'
...
test_expect_success 'exercise strftime with odd fields' '
	echo >expected &&
	git ${GIT_WHICH_TEST} --format="%(authordate:format:)" refs/heads >actual &&
	test_cmp expected actual &&
	long="long format -- $ZERO_OID$ZERO_OID$ZERO_OID$ZERO_OID$ZERO_OID$ZERO_OID$ZERO_OID" &&
	echo $long >expected &&
	git ${GIT_WHICH_TEST} --format="%(authordate:format:$long)" refs/heads >actual &&
	test_cmp expected actual
'
...
test_expect_success 'Verify ascending sort' '
	git ${GIT_WHICH_TEST} --format="%(refname)" --sort=refname >actual &&
	test_cmp expected actual
'
...
test_expect_success 'Give help even with invalid sort atoms' '
	test_expect_code 129 git ${GIT_WHICH_TEST} --sort=bogus -h >actual 2>&1 &&
	grep "^usage: git ${GIT_WHICH_TEST}" actual
'
...
test_expect_success 'exercise patterns with prefixes' '
	...
	git ${GIT_WHICH_TEST} --format="%(refname)" \
...
test_expect_success 'Quoting style: shell' '
	git ${GIT_WHICH_TEST} --shell --format="%(refname)" >actual &&
	test_cmp expected actual
'

test_expect_success 'Quoting style: perl' '
	git ${GIT_WHICH_TEST} --perl --format="%(refname)" >actual &&
	test_cmp expected actual
'

test_expect_success 'Quoting style: python' '
	git ${GIT_WHICH_TEST} --python --format="%(refname)" >actual &&
	test_cmp expected actual
'
...
test_expect_success 'Quoting style: tcl' '
	git ${GIT_WHICH_TEST} --tcl --format="%(refname)" >actual &&
	test_cmp expected actual
'

for i in "--perl --shell" "-s --python" "--python --tcl" "--tcl --perl"; do
	test_expect_success "more than one quoting style: $i" "
		test_must_fail git ${GIT_WHICH_TEST} $i 2>err &&
...
test_expect_success 'Check that :track[short] cannot be used with other atoms' '
	test_must_fail git ${GIT_WHICH_TEST} --format="%(refname:track)" 2>/dev/null &&
	test_must_fail git ${GIT_WHICH_TEST} --format="%(refname:trackshort)" 2>/dev/null
'
...
test_expect_success 'Check for invalid refname format' '
	test_must_fail git ${GIT_WHICH_TEST} --format="%(refname:INVALID)"
'
...
test_expect_success TTY '%(color) shows color with a tty' '
	test_terminal git ${GIT_WHICH_TEST} --format="$color_format" >actual.raw &&
	test_decode_color <actual.raw >actual &&
	test_cmp expected.color actual
'

test_expect_success '%(color) does not show color without tty' '
	TERM=vt100 git ${GIT_WHICH_TEST} --format="$color_format" >actual &&
	test_cmp expected.bare actual
'

test_expect_success '--color can override tty check' '
	git ${GIT_WHICH_TEST} --color --format="$color_format" >actual.raw &&
	test_decode_color <actual.raw >actual &&
	test_cmp expected.color actual
'

test_expect_success 'color.ui=always does not override tty check' '
	git -c color.ui=always ${GIT_WHICH_TEST} --format="$color_format" >actual &&
	test_cmp expected.bare actual
'
...
test_expect_success 'describe atom vs git describe' '
	(
		...
		git ${GIT_WHICH_TEST} --format="%(objectname)" \
			refs/tags/ >obj &&
		while read hash
		do
		...
		git ${GIT_WHICH_TEST} --format="%(objectname) %(describe)" \
...
test_expect_success 'describe:tags vs describe --tags' '
	(
		...
		git ${GIT_WHICH_TEST} --format="%(describe:tags)" \
...
test_expect_success 'describe:abbrev=... vs describe --abbrev=...' '
	(
		...
		git ${GIT_WHICH_TEST} --format="%(describe:abbrev=14)" \
			refs/heads/master >actual &&
		test_cmp expect actual &&
...
test_expect_success 'describe:match=... vs describe --match ...' '
	(
		...
		git ${GIT_WHICH_TEST} --format="%(describe:match="*-foo")" \
			refs/heads/master >actual &&
		test_cmp expect actual
	)
'
...
test_expect_success 'describe:exclude:... vs describe --exclude ...' '
	(
		...
		git ${GIT_WHICH_TEST} --format="%(describe:exclude="*-bar")" \
			refs/heads/master >actual &&
		test_cmp expect actual
	)
'
...
test_expect_success 'deref with describe atom' '
	(
		...
		tagtwo
		EOF
		git ${GIT_WHICH_TEST} --format="%(*describe)" >actual &&
		test_cmp expect actual
	)
'
...
test_expect_success 'err on bad describe atom arg' '
	(
		...
		EOF
		test_must_fail git ${GIT_WHICH_TEST} --format="%(describe:baz)" \
...
test_expect_success 'Check ambiguous head and tag refs (strict)' '
	...
	git ${GIT_WHICH_TEST} --format "%(refname:short)" refs/heads/main refs/tags/main >actual &&
	test_cmp expected actual
'
...
test_expect_success 'Check ambiguous head and tag refs II (loose)' '
	...
	git ${GIT_WHICH_TEST} --format "%(refname:short)" refs/heads/ambiguous refs/tags/ambiguous >actual &&
	test_cmp expected actual
'
...
test_expect_success 'an unusual tag with an incomplete line' '
	...
	git tag -f bogo "$bogo" &&
	git ${GIT_WHICH_TEST} --format "%(body)" refs/tags/bogo
...
test_expect_success GPG 'basic atom: refs/tags/signed-empty raw' '
	...
	git ${GIT_WHICH_TEST} --format="%(raw)" refs/tags/signed-empty >actual &&
	sanitize_pgp <expected >expected.clean &&
	echo >>expected.clean &&
	sanitize_pgp <actual >actual.clean &&
	test_cmp expected.clean actual.clean
...
test_expect_success GPG 'basic atom: refs/tags/signed-short raw' '
	...
	git ${GIT_WHICH_TEST} --format="%(raw)" refs/tags/signed-short >actual &&
	sanitize_pgp <expected >expected.clean &&
	echo >>expected.clean &&
	sanitize_pgp <actual >actual.clean &&
	test_cmp expected.clean actual.clean
...
test_expect_success GPG 'basic atom: refs/tags/signed-long raw' '
	...
	git ${GIT_WHICH_TEST} --format="%(raw)" refs/tags/signed-long >actual &&
	sanitize_pgp <expected >expected.clean &&
	echo >>expected.clean &&
	sanitize_pgp <actual >actual.clean &&
	test_cmp expected.clean actual.clean
...
test_expect_success 'basic atom: refs/mytrees/first raw' '
	...
	echo >>expected &&
	git ${GIT_WHICH_TEST} --format="%(raw)" refs/mytrees/first >actual &&
	test_cmp expected actual &&
	git cat-file -s refs/mytrees/first >expected &&
	git ${GIT_WHICH_TEST} --format="%(raw:size)" refs/mytrees/first >actual &&
	test_cmp expected actual
'
...
test_expect_success 'basic atom: refs/myblobs/first raw' '
	...
	echo >>expected &&
	git ${GIT_WHICH_TEST} --format="%(raw)" refs/myblobs/first >actual &&
	test_cmp expected actual &&
	git cat-file -s refs/myblobs/first >expected &&
	git ${GIT_WHICH_TEST} --format="%(raw:size)" refs/myblobs/first >actual &&
	test_cmp expected actual
'
...
test_expect_success 'Verify sorts with raw' '
	...
	refs/heads/main
	EOF
	git ${GIT_WHICH_TEST} --format="%(refname)" --sort=raw \
...
test_expect_success 'Verify sorts with raw:size' '
	...
	refs/heads/main
	EOF
	git ${GIT_WHICH_TEST} --format="%(refname)" --sort=raw:size \
		refs/heads/main refs/myblobs/ refs/mytrees/first >actual &&
	test_cmp expected actual
'
...
test_expect_success 'validate raw atom with %(if:equals)' '
	...
	not equals
	EOF
	git ${GIT_WHICH_TEST} --format="%(if:equals=abc)%(raw)%(then)%(refname)%(else)not equals%(end)" \
...
test_expect_success 'validate raw atom with %(if:notequals)' '
	...
	refs/myblobs/first
	EOF
	git ${GIT_WHICH_TEST} --format="%(if:notequals=abc)%(raw)%(then)%(refname)%(else)equals%(end)" \
		refs/myblobs/ refs/heads/ >actual &&
	test_cmp expected actual
'
...
test_expect_success 'empty raw refs with %(if)' '
	...
	EOF
	git ${GIT_WHICH_TEST} --format="%(refname) %(if)%(raw)%(then)not empty%(else)empty%(end)" \
		refs/myblobs/ >actual &&
	test_cmp expected actual
'

test_expect_success '%(raw) with --python must fail' '
	test_must_fail git ${GIT_WHICH_TEST} --format="%(raw)" --python
'

test_expect_success '%(raw) with --tcl must fail' '
	test_must_fail git ${GIT_WHICH_TEST} --format="%(raw)" --tcl
'

test_expect_success PERL_TEST_HELPERS '%(raw) with --perl' '
	git ${GIT_WHICH_TEST} --format="\$name= %(raw);
...
test_expect_success '%(raw) with --shell must fail' '
	test_must_fail git ${GIT_WHICH_TEST} --format="%(raw)" --shell
'

test_expect_success '%(raw) with --shell and --sort=raw must fail' '
	test_must_fail git ${GIT_WHICH_TEST} --format="%(raw)" --sort=raw --shell
'

test_expect_success '%(raw:size) with --shell' '
	git ${GIT_WHICH_TEST} --format="%(raw:size)" | sed "s/^/$SQ/;s/$/$SQ/" >expect &&
	git ${GIT_WHICH_TEST} --format="%(raw:size)" --shell >actual &&
	test_cmp expect actual
'

test_expect_success "${GIT_WHICH_TEST} --format compare with cat-file --batch" '
	git rev-parse refs/mytrees/first | git cat-file --batch >expected &&
	git ${GIT_WHICH_TEST} --format="%(objectname) %(objecttype) %(objectsize)
...
test_expect_success 'do not dereference NULL upon %(HEAD) on unborn branch' '
	...
	git ${GIT_WHICH_TEST} --format="%(HEAD) %(refname:short)" refs/heads/ >actual &&
...
test_trailer_option () {
	...
	test_expect_success $prereq "$title" '
		git ${GIT_WHICH_TEST} --format="%($option)" refs/heads/main >actual &&
		test_cmp expect actual &&
		git ${GIT_WHICH_TEST} --format="%(contents:$option)" refs/heads/main >actual &&
		test_cmp expect actual
	'
}
...
test_expect_success 'multiple %(trailers) use their own options' '
	...
	git ${GIT_WHICH_TEST} --format="$t1%0a$t2" refs/tags/tag-with-trailers >actual &&
	cat >expect <<-\EOF &&
	oneWfooXoneWbar
	twoYbazZtwoYqux
	EOF
...
test_failing_trailer_option () {
	...
	test_expect_success "$title" '
		...
		test_must_fail git ${GIT_WHICH_TEST} --format="%($option)" refs/heads/main 2>actual &&
		test_cmp expect actual &&
		test_must_fail git ${GIT_WHICH_TEST} --format="%(contents:$option)" refs/heads/main 2>actual &&
		test_cmp expect actual
	'
}
...
test_expect_success 'if arguments, %(contents:trailers) shows error if colon is missing' '
	...
	EOF
	test_must_fail git ${GIT_WHICH_TEST} --format="%(contents:trailersonly)" 2>actual &&
	test_cmp expect actual
'

test_expect_success 'basic atom: head contents:trailers' '
	git ${GIT_WHICH_TEST} --format="%(contents:trailers)" refs/heads/main >actual &&
	sanitize_pgp <actual >actual.clean &&
	# git ${GIT_WHICH_TEST} ends with a blank line
	cat >expect <<-EOF &&
	$(cat trailers)
...
test_expect_success 'trailer parsing not fooled by --- line' '
	...
	git ${GIT_WHICH_TEST} --format="%(trailers)" refs/heads/main >actual &&
	test_cmp expect actual
'
...
test_expect_success 'Verify usage of %(symref) atom' '
	git ${GIT_WHICH_TEST} --format="%(symref)" refs/heads/sym >actual &&
	test_cmp expected actual
'
...
test_expect_success 'Verify usage of %(symref:short) atom' '
	git ${GIT_WHICH_TEST} --format="%(symref:short)" refs/heads/sym >actual &&
	test_cmp expected actual
'
...
test_expect_success 'Verify usage of %(symref:lstrip) atom' '
	git ${GIT_WHICH_TEST} --format="%(symref:lstrip=2)" refs/heads/sym > actual &&
	git ${GIT_WHICH_TEST} --format="%(symref:lstrip=-2)" refs/heads/sym >> actual &&
	test_cmp expected actual &&

	git ${GIT_WHICH_TEST} --format="%(symref:strip=2)" refs/heads/sym > actual &&
	git ${GIT_WHICH_TEST} --format="%(symref:strip=-2)" refs/heads/sym >> actual &&
	test_cmp expected actual
'
...
test_expect_success 'Verify usage of %(symref:rstrip) atom' '
	git ${GIT_WHICH_TEST} --format="%(symref:rstrip=2)" refs/heads/sym > actual &&
	git ${GIT_WHICH_TEST} --format="%(symref:rstrip=-2)" refs/heads/sym >> actual &&
	test_cmp expected actual
'
...
test_expect_success "${GIT_WHICH_TEST} --ignore-case ignores case" '
	git ${GIT_WHICH_TEST} --format="%(refname)" refs/heads/MAIN >actual &&
	test_must_be_empty actual &&

	echo refs/heads/main >expect &&
	git ${GIT_WHICH_TEST} --format="%(refname)" --ignore-case \
		refs/heads/MAIN >actual &&
	test_cmp expect actual
'

test_expect_success "${GIT_WHICH_TEST} --omit-empty works" '
	git ${GIT_WHICH_TEST} --format="%(refname)" >actual &&
	test_line_count -gt 1 actual &&
	git ${GIT_WHICH_TEST} --format="%(if:equals=refs/heads/main)%(refname)%(then)%(refname)%(end)" --omit-empty >actual &&
	echo refs/heads/main >expect &&
	test_cmp expect actual
'

test_expect_success "${GIT_WHICH_TEST} --ignore-case works on multiple sort keys" '
	...
	git ${GIT_WHICH_TEST} --ignore-case \
...
test_expect_success "git ${GIT_WHICH_TEST} --stdin: empty" '
	>in &&
	git ${GIT_WHICH_TEST} --format="%(refname)" --stdin <in >actual &&
	git ${GIT_WHICH_TEST} --format="%(refname)" >expect &&
	test_cmp expect actual
'

test_expect_success "git ${GIT_WHICH_TEST} --stdin: fails if extra args" '
	>in &&
	test_must_fail git ${GIT_WHICH_TEST} --format="%(refname)" \
		--stdin refs/heads/extra <in 2>err &&
	grep "unknown arguments supplied with --stdin" err
'

test_expect_success "git ${GIT_WHICH_TEST} --stdin: matches" '
	cat >in <<-EOF &&
	refs/tags/multi*
	refs/heads/amb*
	EOF
...
test_expect_success "git ${GIT_WHICH_TEST} with non-existing refs" '
	cat >in <<-EOF &&
	...
	xargs git ${GIT_WHICH_TEST} --format="%(refname)" <in >actual &&
	test_must_be_empty actual
'

test_expect_success "git ${GIT_WHICH_TEST} with nested tags" '
...
test_expect_success 'is-base atom with non-commits' '
	git ${GIT_WHICH_TEST} --format="%(is-base:HEAD) %(refname)" >out 2>err &&
...
test_expect_success GPG2 'bare signature atom' '
	...
	echo  >>expect &&
	git ${GIT_WHICH_TEST} refs/tags/first-signed \
...
test_expect_success GPGSSH 'show good signature with custom format with ssh' '
	...
	git ${GIT_WHICH_TEST} refs/tags/eighth-signed-ssh \
		--format="$GRADE_FORMAT" >actual &&
	test_cmp expect actual
'
...
test_expect_success GPG 'signature atom with grade option and bad signature' '
	...
	EOF
	git ${GIT_WHICH_TEST} refs/tags/third-signed \
		--format="$GRADE_FORMAT" >actual &&
	test_cmp expect actual
'
...
test_expect_success GPG 'show untrusted signature with custom format' '
	...
	EOF
	git ${GIT_WHICH_TEST} refs/tags/fourth-signed \
		--format="$GRADE_FORMAT" >actual &&
	test_cmp expect actual
'
...
test_expect_success GPG 'show untrusted signature with ultimate trust level' '
	...
	73D758744BE721698EC54E8713B6F51ECDDE430D
	EOF
	git ${GIT_WHICH_TEST} refs/tags/sixth-signed \
		--format="$TRUSTLEVEL_FORMAT" >actual &&
	test_cmp expect actual
'
...
test_expect_success GPG 'show unknown signature with custom format' '
	...
	EOF
	GNUPGHOME="$GNUPGHOME_NOT_USED" git ${GIT_WHICH_TEST} \
...
test_expect_success GPG 'show lack of signature with custom format' '
	...
	EOF
	git ${GIT_WHICH_TEST} refs/tags/seventh-unsigned \
		--format="$GRADE_FORMAT" >actual &&
	test_cmp expect actual
'
